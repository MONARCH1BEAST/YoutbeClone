...
export default function WatchLaterContent() {
  ...
  const handleRemoveFromWatchLater = async (watchLaterId: string) => {
    ...
    const item = watchLater.find((savedVideo) => savedVideo._id === watchLaterId);
    const videoId = item?.videoid?._id;

    if (!videoId) {
      setWatchLater(watchLater.filter((savedVideo) => savedVideo._id !== watchLaterId));
      return;
    }
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Unable to remove from Watch later.");
    }
  };
  ...
  const activeWatchLater = watchLater.filter((item) => item?.videoid);

  if (activeWatchLater.length === 0) {
    return (
      <div className="text-center py-12">
        <Clock className="w-16 h-16 mx-auto text-gray-400 mb-4" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-600">{activeWatchLater.length} videos</p>
      ...
      <div className="space-y-4">
        {activeWatchLater.map((item) => (
          <div key={item._id} className="flex gap-4 group">
            <Link href={`/watch/${item.videoid._id}`} className="flex-shrink-0">
              <div className="relative w-40 aspect-video bg-gray-100 rounded overflow-hidden">
                <video
                  src={`${process.env.NEXT_PUBLIC_API_URL}/${item.videoid?.filepath}`}
