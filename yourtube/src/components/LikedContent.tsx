...
export default function LikedVideosContent() {
  ...
  const handleUnlikeVideo = async (videoId: string, likedVideoId: string) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Unable to remove liked video.");
    }
  };
  ...
  const activeLikedVideos = likedVideos.filter((item) => item?.videoid);

  if (activeLikedVideos.length === 0) {
    return (
      <div className="text-center py-12">
        <ThumbsUp className="w-16 h-16 mx-auto text-gray-400 mb-4" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-600">{activeLikedVideos.length} videos</p>
