...
export default function HistoryContent() {
  ...
  const handleRemoveFromHistory = async (historyId: string) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Unable to remove from history.");
    }
  };
  ...
  const activeHistory = history.filter((item) => item?.videoid);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-600">{activeHistory.length} videos</p>
      </div>
